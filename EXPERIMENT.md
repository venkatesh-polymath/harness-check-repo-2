...
Run exactly:
    SH_ARM=none SH_SMOKE=1 python src/run_intervention.py
...
Run exactly this single command and wait until it returns:
    SH_ARM=none python src/run_intervention.py 2>&1 | tee results/iv_none/run.log
This writes results/iv_none/RESULTS.json incrementally and, on completion, a final
...
## Finish
Print results/iv_none/RESULTS.json and stop. Never commit weights.
